#!/usr/bin/env python
"""
ablation_track1_mlp_random.py
-----------------------------
Track 1 ablation with TRAINABLE projection MLPs — random-embedding / label
permutation controls for bias diagnosis.
...
Ablation conditions A–E (original)
----------------------------------
...
Random / bias conditions R1–R4 (default for this script)
--------------------------------------------------------
  R1  Drug original, protein random (Xavier), labels original
  R2  Drug random (Xavier), protein original, labels original
  R3  Drug + protein original, TRAIN labels permuted (val/test unchanged)
      → high test AUROC here indicates spurious feature–label correlations
        (e.g. degree / frequency bias) rather than learned biology
  R4  Drug + protein random (Xavier), labels original  (same inputs as D)
...
-----
  ...
  # Random / bias conditions (default)
  python ablation/ablation_track1_mlp_random.py ... --conditions R1,R2,R3,R4
...
def _make_xavier_noise(n_drugs: int, dim: int, seed: int) -> torch.Tensor:
    """Fixed per-drug Xavier-uniform noise (no pretrained signal)."""
    g = torch.Generator()
    g.manual_seed(seed)
    bound = math.sqrt(3.0 / dim)
    return torch.empty(n_drugs, dim).uniform_(-bound, bound, generator=g)


def _permute_labels(labels: np.ndarray, idx: np.ndarray, seed: int) -> np.ndarray:
    """Shuffle labels at idx in-place copy; indices outside idx are unchanged."""
    out = labels.copy()
    rng = np.random.RandomState(seed)
...
def make_condition_tensors(
    ...
    Conditions B and C zero-out the inactive branch.  D / R4 replace both
    branches with fixed Xavier noise.  R1 randomizes protein only; R2 drug only.
    R3 uses original features (label permutation handled in the training loop).
    Condition E passes chemberta+mono for drug and esm2+ppi for protein.
    """
    drug_dim = drug_raw.shape[1]
    prot_dim = prot_raw.shape[1]
    rand_drug = _make_xavier_noise(n_drugs, drug_dim, xavier_seed)
    rand_prot = _make_xavier_noise(n_drugs, prot_dim, xavier_seed + 1)

    if condition in ("A", "R3"):
    ...
    elif condition in ("D", "R4"):
        return rand_drug, rand_prot, None, None

    elif condition == "R1":
        # Drug original, protein random
        return drug_raw, rand_prot, None, None

    elif condition == "R2":
        # Drug random, protein original
        return rand_drug, prot_raw, None, None
...
def parse_args() -> argparse.Namespace:
    ...
    # Conditions
    p.add_argument("--conditions", default="R1,R2,R3,R4",
                   help="Comma-separated conditions: R1–R4 (random/bias) or A–E (original)")
...
def main() -> None:
    ...
    print("=" * 60)
    print("Track 1 MLP Ablation — random embedding / label permutation controls")
    print("=" * 60)
    ...
    _CONDITION_DESC = {
        "R1": "drug=original, protein=random, labels=original",
        "R2": "drug=random, protein=original, labels=original",
        "R3": "drug=original, protein=original, train labels=permuted",
        "R4": "drug=random, protein=random, labels=original",
        "D":  "same inputs as R4",
    }
    ...
    for condition in conditions:
        print(f"\n{'─'*40}")
        desc = _CONDITION_DESC.get(condition, "")
        print(f"Condition {condition}" + (f"  ({desc})" if desc else ""))
        ...
        # R3: permute train labels only; val/test keep true labels for bias readout
        labels_for_train = labels
        if condition == "R3":
            labels_for_train = _permute_labels(labels, subtrain_idx, seed=args.seed + 103)
            n_pos = int(labels_for_train[subtrain_idx].sum())
            n_neg = len(subtrain_idx) - n_pos
            print(f"  R3: permuted train labels ({n_pos:,} pos / {n_neg:,} neg); "
                  f"val/test labels unchanged")

        def _make_ds(
            idx_sel: np.ndarray,
            label_arr: np.ndarray | None = None,
        ) -> PSEPairDataset:
            y = labels if label_arr is None else label_arr
            return PSEPairDataset(
                s1[idx_sel], s2[idx_sel],
                y[idx_sel], se_ids[idx_sel], degrees[idx_sel],
                drug_feat, prot_feat, mono_feat, ppi_feat,
            )

        train_ds = _make_ds(subtrain_idx, labels_for_train)
