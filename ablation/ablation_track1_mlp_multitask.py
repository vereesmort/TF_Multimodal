#!/usr/bin/env python
"""
ablation_track1_mlp_multitask.py
---------------------------------
Multi-task variant of ablation_track1_mlp.py.

Instead of training one binary classifier per (drug_pair, SE_type) sample, this
script trains a SINGLE model that outputs a logit for EVERY SE simultaneously for
a given drug pair.  The output head is a multi-label sigmoid layer:
...
Key change vs. the binary version
-----------------------------------
  - The dataset now returns one row per DRUG PAIR (not per drug-pair-SE triple).
    The label is a float32 vector of length n_se (1 = SE present, 0 = absent).
  - Loss is binary cross-entropy averaged over all (pair, SE) cells, with per-SE
    positive weights to handle the extreme frequency imbalance between T1 and T5
    side effects.
  - Evaluation still reports per-SE AUROC / AUPRC / AP@50 (same format as
    ablation_track1_mlp.py) so results are directly comparable.

Ablation conditions A–E
------------------------
  Same encoder swap logic as the binary script (see make_condition_tensors).

Usage
-----
  python ablation/ablation_track1_mlp_multitask.py \\
...
def main() -> None:
    ...
    print(
        f"  Split: train={len(subtrain_idx):,}  val={len(val_idx):,}  "
        f"test={len(test_idx):,}"
    )
    ...
    if args.wandb:
        ...
        wandb_run = wandb.init(
            ...
            config=vars(args) | {"n_se": n_se, "n_pairs": n_pairs},
        )

    results_csv = output_dir / "ablation_mlp_mt_results_per_se.csv"
    curves_csv  = output_dir / "ablation_mlp_mt_training_curves.csv"
    first_write = True
    first_curve = True
    wandb_step  = 0
    ...
    for condition in conditions:
        ...
        results_df = evaluate_multitask(lm_t, sm_t, degs_t, active_ses, tier_map, condition)
    ...
    summary.to_csv(output_dir / "ablation_mlp_mt_summary.csv", index=False)
