...
def _sample_random_drug(
    all_drugs: list[str],
    exclude: set[str],
    rng: np.random.RandomState,
) -> str:
    """Sample a random drug not in ``exclude``."""
    while True:
        neg = all_drugs[rng.randint(len(all_drugs))]
        if neg not in exclude:
            return neg


def _sample_degree_matched_drug(
    target_degree: int,
    degree_to_drugs: dict[int, list[str]],
    sorted_degrees: np.ndarray,
    exclude: set[str],
    rng: np.random.RandomState,
    max_degree_diff: int,
) -> str | None:
    """
    Sample a drug with degree close to ``target_degree``.

    Strategy:
    1) exact degree bucket first
    2) nearest degree buckets with absolute difference <= max_degree_diff
    3) return None if no candidate is found
    """
    exact_bucket = [d for d in degree_to_drugs.get(target_degree, []) if d not in exclude]
    if exact_bucket:
        return exact_bucket[rng.randint(len(exact_bucket))]

    if len(sorted_degrees) == 0:
        return None

    ins = int(np.searchsorted(sorted_degrees, target_degree, side="left"))
    left = ins - 1
    right = ins
    n_deg = len(sorted_degrees)

    while left >= 0 or right < n_deg:
        left_diff = abs(int(sorted_degrees[left]) - target_degree) if left >= 0 else np.inf
        right_diff = abs(int(sorted_degrees[right]) - target_degree) if right < n_deg else np.inf
        best_diff = min(left_diff, right_diff)
        if best_diff > max_degree_diff:
            break

        candidate_degrees: list[int] = []
        if left >= 0 and left_diff == best_diff:
            candidate_degrees.append(int(sorted_degrees[left]))
            left -= 1
        if right < n_deg and right_diff == best_diff:
            candidate_degrees.append(int(sorted_degrees[right]))
            right += 1

        for deg in candidate_degrees:
            bucket = [d for d in degree_to_drugs[deg] if d not in exclude]
            if bucket:
                return bucket[rng.randint(len(bucket))]
...
def build_pairs(
    ...
    neg_sampling: str = "degree_match",
    deg_match_max_degree_diff: int = 50,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    Generate (s1_idx, s2_idx, labels, se_ids, degrees) arrays.

    Corrupt one drug per positive (50 % left, 50 % right).

    - random: uniformly sample a replacement drug from the vocabulary
    - degree_match: sample a replacement drug with degree close to the
      corrupted endpoint's degree (fallback to random if no match found)
    ...
    drug_degree  = {d: int(degree.get(d, 1)) for d in all_drugs}
    degree_to_drugs: dict[int, list[str]] = defaultdict(list)
    for d in all_drugs:
        degree_to_drugs[drug_degree[d]].append(d)
    sorted_degrees = np.array(sorted(degree_to_drugs.keys()), dtype=np.int64)
    ...
    for i in range(n_pos):
        ...
        for _ in range(neg_ratio):
            corrupt_left = rng.rand() < 0.5
            exclude = {a, b}

            if neg_sampling == "degree_match":
                target_degree = drug_degree[a] if corrupt_left else drug_degree[b]
                neg = _sample_degree_matched_drug(
                    target_degree=target_degree,
                    degree_to_drugs=degree_to_drugs,
                    sorted_degrees=sorted_degrees,
                    exclude=exclude,
                    rng=rng,
                    max_degree_diff=deg_match_max_degree_diff,
                )
                if neg is None:
                    n_degmatch_fallback_random += 1
                    neg = _sample_random_drug(all_drugs, exclude, rng)
            else:
                neg = _sample_random_drug(all_drugs, exclude, rng)
    ...
    if neg_sampling == "degree_match":
        total_negs = n_pos * neg_ratio
        print(
            "  Degree-match negative sampling: "
            f"fallback-to-random {n_degmatch_fallback_random:,}/{total_negs:,} "
            f"({100.0 * n_degmatch_fallback_random / max(total_negs, 1):.2f}%)"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--neg_sampling",
        choices=["degree_match", "random"],
        default="degree_match",
        help="Negative sampling strategy: degree-aware matching or random corruption",
    )
    p.add_argument(
        "--deg_match_max_degree_diff",
        type=int,
        default=50,
        help="Maximum allowed absolute degree difference for degree-matched negatives",
    )
    ...
    if args.wandb:
        ...
        wandb_run = wandb.init(
            ...
            config={
                ...
                "deg_match_max_degree_diff": args.deg_match_max_degree_diff,
